...
SUPABASE_PROJECT_URL="${SUPABASE_PROJECT_URL:-${NEXT_PUBLIC_SUPABASE_URL:-}}"
...
Define SUPABASE_REST_URL y SUPABASE_ANON_KEY, o usa NEXT_PUBLIC_SUPABASE_URL y NEXT_PUBLIC_SUPABASE_ANON_KEY en web/.env.
EOF
  exit 1
fi
...
response=$(curl --fail-with-body -sS -X POST "$SUPABASE_REST_URL" \
